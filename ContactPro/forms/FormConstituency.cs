...
{
    public partial class FormConstituency : Office2007Form
    {
        Conn conn = new Conn();
        ...
        private void FormConstituency_Load(object sender, EventArgs e)
        {
            labelMode.Text = "Mode: New Entry";
            FRMENTMODE = "New";
            clear();
        ...
        private void FormConstituency_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        {
            Conn.Open();
            cmd = new MySqlCommand("SELECT id, code, name FROM tbl_constituency WHERE code = @code", Conn.allcon);
        ...
        public void auto()
        {
            Conn.Open();
            int autonum = 0;
            cmd = new MySqlCommand("SELECT MAX(id+0) FROM tbl_constituency", Conn.allcon);
            dr = cmd.ExecuteReader();
        ...
        private void save()
        {
            ...
            else
            {
                if (FRMENTMODE == "New")
                {
                    cmd = new MySqlCommand("INSERT INTO tbl_constituency (id, code, name) VALUES (@id, @code, @name)", Conn.allcon);
        ...
        {
            Conn.Open();
            cmd = new MySqlCommand("UPDATE tbl_constituency SET name = @name WHERE code = @code", Conn.allcon);
        ...
        public void deldata()
        {
            ...
            if (confirm == DialogResult.Yes)
            {
                cmd = new MySqlCommand("DELETE FROM tbl_constituency WHERE code = @code", Conn.allcon);
                cmd.Parameters.AddWithValue("@code", txt_code.Text);
                try
                {
                    cmd.ExecuteNonQuery();
        ...
        public void districtdata()
        {
            ...
            ds = new DataSet();
            da = new MySqlDataAdapter("SELECT id, code, name FROM tbl_constituency WHERE name LIKE '" + txtsrch.Text + "%' ORDER BY name;", Conn.allcon);
            da.Fill(ds, "district");
