...
{
    public partial class FormPanchayat : Office2007Form
    {
        Conn conn = new Conn();
        ...
        private void FormPanchayat_Load(object sender, EventArgs e)
        {
        ...
        private void FormPanchayat_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        {
            Conn.Open();
            cmd = new MySqlCommand("SELECT id, code, name FROM tbl_panchayat WHERE code = @code", Conn.allcon);
        ...
        public void auto()
        {
            Conn.Open();
            int autonum = 0;
            cmd = new MySqlCommand("SELECT MAX(id+0) FROM tbl_panchayat", Conn.allcon);
            dr = cmd.ExecuteReader();
        ...
        private void save()
        {
            ...
            else
            {
                if (FRMENTMODE == "New")
                {
                    cmd = new MySqlCommand("INSERT INTO tbl_panchayat (id, code, name) VALUES (@id, @code, @name)", Conn.allcon);
        ...
        {
            Conn.Open();
            cmd = new MySqlCommand("UPDATE tbl_panchayat SET name = @name WHERE code = @code", Conn.allcon);
        ...
        public void deldata()
        {
            ...
            if (confirm == DialogResult.Yes)
            {
                cmd = new MySqlCommand("DELETE FROM tbl_panchayat WHERE code = @code", Conn.allcon);
                cmd.Parameters.AddWithValue("@code", txt_code.Text);
                try
                {
                    cmd.ExecuteNonQuery();
        ...
        public void districtdata()
        {
            ...
            ds = new DataSet();
            da = new MySqlDataAdapter("SELECT id, code, name FROM tbl_panchayat WHERE name LIKE '" + txtsrch.Text + "%' ORDER BY name;", Conn.allcon);
            da.Fill(ds, "district");
